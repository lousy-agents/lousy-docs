...
- Schema validates each inventory item has `id`, `title`, `summary`, `primaryDocsHref` (must match `^/docs/[^/]+$`), `primaryContentSlug`, optional `fallbackDocsHref`, and optional `fallbackContentSlug`.
