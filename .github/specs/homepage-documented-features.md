...
**Affected files**:
- `src/entities/feature.ts` *(new — plain TypeScript types only, no Zod: `HomepageFeatureInventoryItem`, `ResolvedHomepageFeature`)*
- `src/use-cases/select-available-features.ts` *(new — pure selector function, no Zod imports, operates on entity types imported from `src/entities/feature.ts`)*
...
- `package.json` *(add `"fixture:docs-slugs"` script — an ES module that resolves `src/content/docs/` using `fileURLToPath(new URL('../src/content/docs', import.meta.url))`, strips `.md` extensions, writes the resulting slug array as JSON to `tests/fixtures/docs-slugs.json`, and exits with code 1 if the directory is not found — no Astro build required, but `bash scripts/fetch-docs.sh` must be run first in a fresh checkout where `src/content/docs/` has not yet been created)*
...
- `src/entities/feature.ts` exports only plain TypeScript types (`HomepageFeatureInventoryItem`, `ResolvedHomepageFeature`) — no Zod imports, preserving Clean Architecture entities-layer rules.
- `src/use-cases/select-available-features.ts` exports only the `selectAvailableFeatures` function — no Zod imports; the selector operates on entity types from `src/entities/feature.ts`.
...
- Add `"fixture:docs-slugs"` script to `package.json` pointing to an ES module script that resolves `src/content/docs/` using `fileURLToPath(new URL('../src/content/docs', import.meta.url))` (imported from `node:url`), strips `.md` extensions, writes the resulting string array as JSON to `tests/fixtures/docs-slugs.json`, and exits with code 1 if the directory does not exist; commit the generated fixture file to source control.
