...
Shared engineering standards for the lousy-docs repository. This file is the source of record for every rule that is not specific to one agent harness.

Harnesses load it differently, which is why the wrappers exist:

- **Codex / OpenCode** read this file directly from the repository root.
- **Claude Code** reads `CLAUDE.md`, which imports this file with a native `@./AGENTS.md` token and adds what is specific to that harness.
- **GitHub Copilot** reads `.github/copilot-instructions.md`. Copilot does not expand `@` import tokens, so that file deliberately restates the rules below rather than pointing at them. When you change a rule here, change it there in the same commit.
